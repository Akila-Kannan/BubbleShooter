...
public class Shooter : MonoBehaviour
{
	...
	public void Update()
	{
		...
		{
			gizmosPoint = Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue());
			lookDirection = Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue()) - transform.position;
			lookAngle = Mathf.Atan2(lookDirection.y, lookDirection.x) * Mathf.Rad2Deg;

			if (Mouse.current.leftButton.wasReleasedThisFrame
				&& (Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue()).y > bottomShootPoint.transform.position.y)
				&& (Camera.main.ScreenToWorldPoint(Mouse.current.position.ReadValue()).y < limit.transform.position.y))
			{
